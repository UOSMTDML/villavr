using UnityEngine;
using FaustUtilities_SAM_VirtualAnalog;
...
public class FaustPlugin_SAM_VirtualAnalog: FaustObject { // ADDED INHERITANCE
	// Array containing the value of each parameter, has to be public to be serialized by the Unity engine
	public float[] parameters = new float[45];
    ...
    * @param x New parameter value */
	public override void setParameter(int param, float x) {
		if (IsInstantiated()) {
    ...
    */
	...
	{
		// Generates sound, so no input handling needed 
		ctx.process(buffer, buffer.Length / numchannels, numchannels);
	}
	// ADDED 
